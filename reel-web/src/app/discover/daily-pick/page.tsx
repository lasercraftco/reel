...
export default async function DailyPickPage(): Promise<React.ReactElement> {
  const user = await readSessionFromCookie();
  if (!user) redirect("/auth");
  const data = await engine<{ movie: Recommendation["movie"]; reason: string }>("/api/smart/daily-pick");
  ...
  return (
    <main className="min-h-dvh">
      ...
      <section className="px-4 sm:px-6 lg:px-10 pb-20 max-w-[1700px] mx-auto pt-6 space-y-6">
        <h1 className="text-2xl font-semibold">Today's pick</h1>
        <HeroStrip recs={[rec]} />
      </section>
    </main>
  );
